...
class DNSZone(script.Script):
    def __init__(self, zone, where="", port=53, source=None, source_port=0, timeout=None, lifetime=None, use_udp=False,
        ...
        if keyalgorithm and keyalgorithm not in [alg for alg in dir(dns.tsig) if alg.startswith("HMAC_")]:
            raise ValueError("Unknown or invalid TSIG Algorithm \"{}\" ...".format(keyalgorithm))
    ...
    def transfer(self):
        try:
            ...
            names = sorted(list(zone.nodes.keys()))
            self.print("[i] Successfully transfered the DNS Zone from domain \"{}\":".format(self.name))
            if self.fqdn not in self.address_list: self.print("    {} {}".format(self.colored("Fully Qualified Domain Name: "), self.colored(self.fqdn, dark=True)))
            self.print("    {} {}".format(self.colored("Domain Name: "), self.colored(self.name, dark=True)))
            if self.address_list: self.print("    {} {}".format(self.colored("{}: ".format("Address List" if len(self.address_list) > 1 else "IP Address")), self.colored("\n        ".join(self.address_list), dark=True)))
            if self.alias_list: self.print("    {} {}".format(self.colored("Alias List: "), self.colored("\n        ".join(self.address_list), dark=True)))
            self.print("")
            for name in names:
                self.print("    {}".format(zone[name].to_text(name).replace("\n", "\n    ")), dark=True)
        except KeyboardInterrupt as e:
            self.print("[!] {}".format(str(e)), "red")


parser = argparse.ArgumentParser("DNSZoneTransfer", description="Requests a zone transfer (AXFR) from a DNS server.")
...
parser.add_argument("--af", type=str, default="AF_INET", help="the address family to use. The default is None, which causes the address family to use to be inferred from the form of where. If the inference attempt fails, AF_INET is used.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if str(e) == "%u format: a number is required, not NoneType":
            print("[!] Please specify a serial using --serial in order to complete the request ...")
    except Exception as e:
        print("[!] Failed to retrieve the DNS Zone from \"{}\":".format(args["zone"]))
        print("    " + str(e))
